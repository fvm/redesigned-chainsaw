...
So, suppose you have a lanternfish with an uint64ernal timer value of 3:

- After one day, its uint64ernal timer would become 2.
- After another day, its uint64ernal timer would become 1.
- After another day, its uint64ernal timer would become 0.
- After another day, its uint64ernal timer would reset to 6, and it would create a new lanternfish with an uint64ernal
	timer of 8.
- After another day, the first lanternfish would have an uint64ernal timer of 5, and the second lanternfish would have
	an uint64ernal timer of 7.

A lanternfish that creates a new fish resets its timer to 6, not 7 (because 0 is included as a valid timer value). The
new lanternfish starts with an uint64ernal timer of 8 and does not start counting down until the next day.
...
This list means that the first fish has an uint64ernal timer of 3, the second fish has an uint64ernal timer of 4, and so
on until the fifth fish, which has an uint64ernal timer of 2. Simulating these fish over several days would proceed as
follows:
...
Your puzzle answer was 1622533344325.
